...
<!-- 
	...
	YOU NEED TO FILL IN YOUR OWN FILE SERVER URL ON LINE 1459.
 -->
			...
			<xsl:for-each select="DISS_submission">
				<document>
					...
					<fulltext-url>
							...
							<xsl:value-of
								select="concat('http://lamp.lib.iastate.edu/DR/r20150924/', $pdfpath)"/>
					...
					<department>
						<xsl:for-each select="DISS_description/DISS_institution/DISS_inst_contact">
							...
							<xsl:choose>
								...
								<xsl:when test=".='Integrated Visual Arts'">
									<xsl:value-of>Art and Visual Culture</xsl:value-of>
								</xsl:when>
